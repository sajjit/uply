...
import { ClipboardList, Calendar, FileText, Download } from 'lucide-react';
import * as api from '../../lib/api';
import { generateInvoicePdf, generatePurchaseOrderPdf, downloadPdf } from '../../lib/pdf/generatePdf';
...
export default function AdminOrders({ orders, restaurants, onChange }) {
  ...
  async function sendInvoice(order, restaurantName) {
    setSendingInvoice(order.id);
    try {
      const blob = await generateInvoicePdf(order, restaurantName);
  ...
  async function downloadPurchaseOrder(order, restaurantName) {
    const blob = await generatePurchaseOrderPdf(order, restaurantName);
              ...
              <div key={o.id} style={{ background: '#fff', border: '1.5px solid #E1E6E1', borderRadius: 8, padding: 14 }}>
                ...
                {editingDelivery === o.id ? (
                  <div style={{ display: 'flex', gap: 6, marginBottom: 10, alignItems: 'center' }}>
                    <input type="date" value={deliveryForm.date} onChange={(e) => setDeliveryForm({ ...deliveryForm, date: e.target.value })} style={{ ...inputStyle, flex: 1 }} />
                    <input placeholder={t('deliveryWindowPlaceholder')} value={deliveryForm.window} onChange={(e) => setDeliveryForm({ ...deliveryForm, window: e.target.value })} style={{ ...inputStyle, flex: 1 }} />
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                  {STATUS_FR_VALUES.map((st) => (
                    <button
                      ...
                      style={{
                        flex: 1, fontSize: 11, padding: '6px 4px', borderRadius: 4, fontWeight: 600,
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    onClick={() => downloadPurchaseOrder(o, restaurant?.name || '')}
                    style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: '#fff', border: '1px solid #CBD3CB', borderRadius: 6, padding: 8, fontSize: 11, fontWeight: 600, color: '#0D0F0D' }}
                  >
                    <Download size={13} /> Bon de commande
                  </button>
                  {o.status === 'Livrée' && (
                    <button
                      onClick={() => sendInvoice(o, restaurant?.name || '')}
                      disabled={sendingInvoice === o.id}
                      style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: o.invoice_url ? '#5A9C3E' : '#1FB9D6', color: '#fff', border: 'none', borderRadius: 6, padding: 8, fontSize: 11, fontWeight: 600 }}
                    >
