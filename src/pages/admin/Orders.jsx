...
import { SectionHeader, EmptyState, StatusTab, STATUS_FR_VALUES, fmtDate, inputStyle, Stepper } from '../../components/shared';
...
export default function AdminOrders({ orders, restaurants, onChange, archived = false }) {
  ...
  async function changeDeliveredQty(item, qty) {
    await api.setOrderItemDeliveredQty(item.id, qty);
    onChange();
  }
                ...
                {archived ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 10 }}>
                    {(o.order_items || []).map((it) => {
                      ...
                      const qtyChanged = it.prepared && it.delivered_qty != null && it.delivered_qty !== it.qty;
                      return (
                        <div
                          key={it.id}
                          style={{
                        ...
                        >
                          <button
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0, background: 'none', border: 'none', textAlign: 'left', padding: 0 }}
                          >
                            <span style={{
                            ...
                            <span style={{ color: it.prepared ? '#576257' : '#0D0F0D', textDecoration: it.prepared ? 'line-through' : 'none' }}>
                              {it.name} ({it.qty} {it.unit}){qtyChanged ? ` → ${deliveredQty} ${it.unit}` : ''}
                            </span>
                          </button>
                          {it.prepared && (
                            <Stepper value={deliveredQty} onChange={(v) => changeDeliveredQty(it, v)} />
                          )}
                        </div>
                      );
                    })}
