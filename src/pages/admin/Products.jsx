...
export default function AdminProducts({ restaurants, products, onChange }) {
  const { t } = useLanguage();
  const [form, setForm] = useState({ name: '', restaurant_id: restaurants[0]?.id || '', category: '', unit: '', supplier: '', price: '', photo: '📦' });
  ...
  async function addProduct() {
    ...
    setForm({ ...form, name: '', category: '', unit: '', supplier: '', price: '' });
    onChange();
  }
  ...
  function startEdit(p) {
    setEditing(p.id);
    setEditForm({ name: p.name, category: p.category, unit: p.unit, supplier: p.supplier, price: p.price });
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <div style={{ flex: 1, background: '#fff', border: '1.5px solid #0D0F0D', borderRadius: 8, padding: 14 }}>
        ...
        <button
          ...
          style={{ flexShrink: 0, width: 110, background: '#fff', border: '1.5px dashed #8A938A', borderRadius: 8, padding: 10, color: '#576257', fontSize: 11, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, textAlign: 'center' }}
        >
          ...
          <div key={r.id} style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {ps.map((p) => {
                ...
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1px solid #E1E6E1', borderRadius: 8, padding: 10, opacity: p.active ? 1 : 0.45 }}>
