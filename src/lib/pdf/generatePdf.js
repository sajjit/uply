...
function drawLineItemsTable(pdfDoc, page, font, fontBold, items, startY, { showPrice, showRef }) {
  ...
  for (const [i, item] of items.entries()) {
    ...
    currentPage.drawRectangle({ x: MARGIN, y: y - 4, width: PAGE_WIDTH - MARGIN * 2, height: 20, color: rowBg });
    currentPage.drawText(String(item.name).slice(0, 40), { x: cols.colName + 4, y: y, size: 9, font, color: rgb(0.1, 0.1, 0.1) });
    currentPage.drawText(String(item.qty), { x: cols.colQty, y: y, size: 9, font, color: rgb(0.1, 0.1, 0.1) });
    currentPage.drawText(String(item.unit || ''), { x: cols.colUnit, y: y, size: 9, font, color: rgb(0.1, 0.1, 0.1) });
    if (showPrice) {
  ...
  // Leave room for trailing content (total, comment, signature box) — if
  // there's not enough space left on this page, start a fresh one for it.
  if (y < 140) {
    currentPage = pdfDoc.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
    y = PAGE_HEIGHT - MARGIN;
  }

  return { page: currentPage, y, total };
}
...
export async function generateInvoicePdf(order, restaurant) {
  ...
  const { page: lastPage, y: afterTable, total } = drawLineItemsTable(pdfDoc, page, font, fontBold, items, y, { showPrice: true });

  let finalY = afterTable - 10;
  lastPage.drawLine({ start: { x: 330, y: finalY }, end: { x: PAGE_WIDTH - MARGIN, y: finalY }, thickness: 0.5, color: rgb(0.85, 0.83, 0.78) });
  finalY -= 20;
  lastPage.drawText('Total', { x: 390, y: finalY, size: 11, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  lastPage.drawText(`${total.toFixed(2)} €`, { x: 460, y: finalY, size: 11, font: fontBold, color: rgb(0.91, 0.44, 0.23) });

  if (order.comment) {
    finalY -= 40;
    lastPage.drawText('Commentaire :', { x: MARGIN, y: finalY, size: 9, font: fontBold, color: rgb(0.3, 0.3, 0.3) });
    lastPage.drawText(String(order.comment).slice(0, 90), { x: MARGIN, y: finalY - 14, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
  }

  lastPage.drawText('Document généré automatiquement par Uply', { x: MARGIN, y: 40, size: 7, font, color: rgb(0.6, 0.6, 0.6) });
...
export async function generatePurchaseOrderPdf(order, restaurant) {
  ...
  if (order.delivery_date) {
    lastPage.drawText('Livraison prévue :', { x: MARGIN, y: finalY, size: 9, font: fontBold, color: rgb(0.3, 0.3, 0.3) });
    ...
    lastPage.drawText(deliveryText, { x: 170, y: finalY, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
    finalY -= 18;
  }
  if (order.comment) {
    lastPage.drawText('Commentaire :', { x: MARGIN, y: finalY, size: 9, font: fontBold, color: rgb(0.3, 0.3, 0.3) });
...
export async function generateDeliveryNotePdf(order, restaurant) {
  ...
  lastPage.drawRectangle({ x: MARGIN, y: finalY - 70, width: 200, height: 60, borderColor: rgb(0.75, 0.75, 0.75), borderWidth: 1, color: rgb(1, 1, 1) });
